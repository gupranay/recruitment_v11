...
const RecruitmentCycleContext = createContext<
  RecruitmentCycleContextProps | undefined
>(undefined);
...
export const useRecruitmentCycle = () => {
  ...
  if (!context) {
    throw new Error(
      "useRecruitmentCycle must be used within a RecruitmentCycleProvider"
    );
  }
  return context;
};

export const RecruitmentCycleProvider = ({
  children,
}: {
  children: React.ReactNode;
}) => {
  const [selectedRecruitmentCycle, setSelectedRecruitmentCycle] =
    useState<RecruitmentCycle | null>(null);

  return (
    <RecruitmentCycleContext.Provider
      value={{ selectedRecruitmentCycle, setSelectedRecruitmentCycle }}
    >
      {children}
    </RecruitmentCycleContext.Provider>
  );
